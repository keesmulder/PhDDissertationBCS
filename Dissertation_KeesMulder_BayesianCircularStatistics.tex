...
Second, Bayesian algorithms such as MCMC allow us to trivially obtain inference for any function of parameters. This can be exploited to fit a model using parameters which are easy to define, while providing results for parameters that are interpretable (for example, this is used for the Batschelet-type models of Chapter \ref{flexcmix}).

Finally, some circular models are of such mathematical form that it is hard to obtain frequentist standard errors, while the Bayesian approach naturally provides uncertainties for all parameters.

\section{Aims}

The goal of this dissertation is to develop Bayesian methods for circular statistics which solve practical research problems. To this end, it has been a goal to develop collaborations with researchers and organizations working in domains which encounter circular data.
