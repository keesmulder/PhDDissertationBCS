...
\begin{figure}
<<birthday_example, echo = FALSE>>=
dates1 <- data_frame(x = ymd(c("2019-02-03", "2019-02-07", "2019-02-26")), y = 0)
dates2 <- data_frame(x = c(ymd("2019-01-07"), dates1$x), y = 0)
dates3 <- data_frame(x = c(ymd(c("2019-12-05", "2019-12-06", "2019-12-16", "2019-12-26")), dates2$x), y = 0)
...
ylinemax <- 0
direction <- "both"
p1 <- p + geom_text_repel(data = dates1, direction = direction,
...
p2 <- p + geom_text_repel(data = dates2, direction = direction,
                          mapping = aes(x, y, label = format(x, "%b %d")),
                          hjust = hjust, vjust = vjust, angle = angle) +
...
p3 <- p + geom_text_repel(data = dates3, direction = direction,
                          mapping = aes(x, y, label = format(x, "%b %d")),
                          hjust = hjust, vjust = vjust, angle = angle) +
...
\begin{figure}
<<birthday_example, echo = FALSE>>=
p + geom_point(data = dates3, mapping = aes(x, y)) + coord_polar()
