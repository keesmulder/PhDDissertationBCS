...
add_near_line <- list(
  c("\\end{abstract}", "\\newpage", 0),
  c("\\begin{tabular}{llrrrrrrlrrrrrr}", "\\begin{tiny} \\centerline{", -1),
  c("\\begin{tabular}{llrrrrrrlrrrrrr}", "} \\end{tiny}", 22),
  c("Model & Fit function & MCMC & Marginal Likelihood  \\\\ \\hline", "\\begin{scriptsize} \\centerline{", -2),
  c("Model & Fit function & MCMC & Marginal Likelihood  \\\\ \\hline", "} \\end{scriptsize}", 11),
